...
- The underlying is held as `object` (not `StreamWrapper`) on purpose, so a **foreign
  wrapper that only duck-types the protocol** (without extending `StreamWrapper`) is
  accepted at runtime.
